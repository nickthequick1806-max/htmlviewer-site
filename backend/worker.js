...
function createVoiceSystemInstruction(editorContext, language, hasIntroduced){
  ...
  return [
    ...
    'request without claiming it is complete. The application will autonomously improve',
    'the stabilized request into a production implementation prompt, show that prompt in',
    'chat, send it to the dedicated Gemini coding pipeline, insert the complete result',
    'into the editor, and open the Pop Out preview unless the user explicitly says not to.',
    'If that same coding request explicitly asks to push, publish, or deploy the code live,',
    'the application will open GitHub publishing and activate the final publish action',
    'after the generated code and preview steps finish.',
    'Give short spoken progress acknowledgements while that workflow runs. Never ask for',
    'confirmation between those steps and never claim an edit was applied until the',
    'coding pipeline confirms it.',
    ...
    'Image generation is available in this application through FLUX.2 Klein 4B. When the',
    'user asks to create or generate an image, respond positively and briefly—such as',
    '"Okay, coming right up"—because the application will route the final prompt to FLUX.',
    'Never say that you cannot generate images and never try to use the text model for',
    'the image itself.',
    '',
